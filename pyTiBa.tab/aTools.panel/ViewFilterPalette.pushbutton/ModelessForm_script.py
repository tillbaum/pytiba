...
class EventHandler(IExternalEventHandler):
    ...
    def Execute(self, uiapp):
        ...
        try:
             # "running Execute method of SimpleEventHandler"
            self.do_this()
...
handler2 = EventHandler(addFilter)
handler3 = EventHandler(removeFilter)

ext_event_button_select = ExternalEvent.Create(handler1)  
ext_event_addfilter = ExternalEvent.Create(handler2)
ext_event_removefilter = ExternalEvent.Create(handler3)
...
class SelectFromList2(TemplateUserInputWindowModeless):
    ...
    def button_addfilter(self, sender, args):    #pylint: disable=W0613
        ...
        viewfilterId = doc.ActiveView.GetFilters()
        filter2 =[i for i in fecfilter if i.Id not in viewfilterId ]
        ops = sorted([FilterOption(x) for x in filter2], key= lambda x: x.name)
        global response3
        response3 = forms.SelectFromList.show(ops,
                             multiselect=True, 
                             width = 400,
                             height = 900,
                             button_name='Select Item')
        ...
        if response3:
            ext_event_addfilter.Raise() 
            self._context =  sorted([option(x) for x in doc.ActiveView.GetFilters()], key= lambda x: x.name) 
            self.list_lb.ItemsSource = [x for x in self._get_active_ctx()]

    def button_removefilter(self, sender, args):
        filtersinview = doc.ActiveView.GetFilters()
        ops = sorted([FilterOption(doc.GetElement(x)) for x in filtersinview], key= lambda x: x.name)
        global response3
        response3 = forms.SelectFromList.show(ops,
                             multiselect=True, 
                             width = 350,
...
res = SelectFromList2.show(sorted([option(x) for x in doc.ActiveView.GetFilters()], key= lambda x: x.name), 
                               multiselect=True,  
                               return_all = True, 
                               width = 300,
                               height = 600,
