"""	- Active Sheet Selection in ProjectBrowser
	  or, if nothing selected,
    - Selection in Sheet-Selection-Dialog
	- Printer: Adobe PDF, """
 
__title__ = "PDFOut_Sheets"
...
# DELETE tempViewSheetSet ViewSet -
...
for i,v in enumerate(viewlist): 
	...
	cntr =0 
	while shsize_str not in dic_ps and cntr < 5: 
...
def printview(doc, sheet, filepath , papersizeobj, pdfprinterName, \
				combined, tmp_printsettingf): 
	...
	try: 
		...
		t.RollBack()
		import time 
		time.sleep(1) # 3 sec, necessary because PDF Printer will cause error when many docs submitted to fast.
...
print("\nViewlist -----------------------------------")
for i in viewlist: 
	print("{} - {}".format(i.SheetNumber, i.Name) 
...
print("\nPaperSize ----------------------------------")
for i in psmess: print(i)
print("----------------------------------------------")

# fmt = '{:<16}  {:<35}  {}'
# print(fmt.format( " Viewlist---------", " FilePathList------------", "PaperSizeList-----------------"))
# print ""
# for v,fp, ps in zip(viewlist, filenamelist, psmess):
