...
def matchPaperSize(viewlist, pdfPrinterName):
	...
	for i,v in enumerate(viewlist): 
		...
		filter_double_rule = \
					...
					FilterNumericGreater(), 0.21 / 0.3048, 1E-2)
		...
		global val_wi, val_hei
		val_wi =  int(FECtb[0].get_Parameter(bip_shwi).AsDouble() * 0.3048 * 1000) #from m to mm
		val_hei = int(FECtb[0].get_Parameter(bip_shhei).AsDouble() * 0.3048 * 1000)

		print val_wi, val_hei, round_up(val_wi, 10), round_up(val_hei, 10)
		wi = int(round_up(val_wi, 10) /10)  # from m to cm 
		hei = int(round_up(val_hei, 10) /10)
		print "int ", wi, hei
