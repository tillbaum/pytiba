...
mode = "a+" # read, create, write pointer at end.
...
# current Active DWGExport Options --------------------------
firstdwgsetting = FilteredElementCollector(doc).OfClass(ExportDWGSettings).FirstElement()
currentactiveset= firstdwgsetting.GetActivePredefinedSettings(doc)
dwgopt= currentactiveset.GetDWGExportOptions()

# fun Export DWG ---------------------------------------------
def ExportDwg(filename, view, folder, dwgoption): 
	#options = DWGExportOptions()
	views = List[ElementId]()
	views.Add(view.Id)
	result = doc.Export(folder, filename, views, dwgoption)
	return result 
...
try:
	...
	for fn, v in zip(fnlist, viewlist):
		ExportDwg(fn, v, filepath, dwgopt)
		print("Sucess")
except:
