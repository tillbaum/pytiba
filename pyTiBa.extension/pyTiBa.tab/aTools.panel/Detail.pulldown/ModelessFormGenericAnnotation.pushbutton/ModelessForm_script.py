...
__doc__ = "Palette GenericAnnotation"
__title__ = "Palette\nGenericAnnotation"
...
def firstrun():
    ...
    genanno = FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_GenericAnnotation) \
                                .WhereElementIsElementType().ToElements()
    
    genanno = [i for i in genanno if not any([i.FamilyName.Contains("SOFiSTiK")])]


    typelist = sorted([(i.FamilyName + " - " + i.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString(), i) for i in genanno], 
                    key = lambda x: x[0] )
    return typelist
