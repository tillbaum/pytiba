...
def createobj(type): 
    try:
        ...
        typegroup = DB.ElementTypeGroup.TextNoteType #ElementTypeGroup Enumeration 
        #famCategoryId = type.Category.Id,              #for Collums - External Categories 
        
        t = Transaction(doc, "TextNote")
        t.Start()
        ...
        # Postable Command 
        revitcomid= UI.RevitCommandId.LookupPostableCommandId(UI.PostableCommand.TextNote)
        if revitcomid:
...
# def firstrun():
    ...
    # if not notes or not len(notes) == len(notetypes):       
        ...
        # for i, j in enumerate(notetypes):
            # text = j.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
            # textnote = DB.TextNote.Create(doc, draftview.Id, XYZ(0,-i*3.5, 0), text , j.Id)
        # t.Commit()    


    # return  [(i.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString(), i) for i in notetypes] # Tupel
...
class ModelessForm(WPFWindow):
    ...
    def __init__(self, xaml_file_name, buttonlist):
        ...
        self.text1.Text = "TextNotes (#" + str(len(typelist)) + ")"
