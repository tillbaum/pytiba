...
try: 								
	...
	forms.alert( "Excel not installed", ok=True )
...
def filter_excel_data(data_list):
    ex_rowfilter = []
    for i in data_list: 
        if i[0] and i[1] and not [k for k in DB.FilteredElementCollector(doc) \
                                        .OfClass(DB.Level) if k.Name == i[0]]:
...
try:
    ...
    forms.alert("ExcelApplication not open", ok=True)
    sys.exit()
except:
    sys.exit()
...
ceilingplantype = [k for k in FECviewfamtype if k.FamilyName == "Structural Plan"][0] 


try: 
    if ex_row[1] or ex_row[2] or ex_row[3]:
        pass
except:
    forms.alert("No Data in Excelsheet found", ok=True)
    sys.exit()
...
for i in ex_row: 
    try: 
        ...
        #if __shiftclick__:
        print("Level ", lev.Name, "Floorplan created" if i[2] else "", "Structuralplan created" if i[3] else "")
    except:
        import traceback
        print(traceback.format_exc())
        continue
