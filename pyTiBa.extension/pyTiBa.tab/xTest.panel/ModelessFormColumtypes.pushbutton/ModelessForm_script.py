...
__doc__ = "Palette StructuralColumn"
__title__ = "Palette\nStructuralColumn"
...
def createobj(type): 
    try:
        ...
        ## typegroup = DB.ElementTypeGroup.WallType #ElementTypeGroup Enumeration 
        famCategoryId = type.Category.Id
        
        t = Transaction(doc, "create Wall")
        t.Start()
        #doc.SetDefaultElementTypeId(famCategoryId, type.Id)
        doc.SetDefaultFamilyTypeId(famCategoryId, type.Id)
        t.Commit()
        
        # Postable Command
        revitcomid= UI.RevitCommandId.LookupPostableCommandId(UI.PostableCommand.StructuralColumn)
        if revitcomid:
...
def firstrun():
    ...
    types = FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_StructuralColumns) \
...
class ModelessForm(WPFWindow):
    ...
    def __init__(self, xaml_file_name, buttonlist):
        ...
        self.text1.Text = "Columns (#" + str(len(typelist)) + ")"
