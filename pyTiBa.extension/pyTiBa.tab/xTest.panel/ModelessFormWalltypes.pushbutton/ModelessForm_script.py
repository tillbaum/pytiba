...
__doc__ = "Palette Walls"
__title__ = "Palette\nWalls"
__author__ = "Tillmann Baumeister"
__persistentengine__ = True


def createobj(walltype): 
    try:
        walltypegroup = DB.ElementTypeGroup.WallType #ElementTypeGroup Enumeration 
        
        t = Transaction(doc, "create Wall")
        t.Start()
        doc.SetDefaultElementTypeId(walltypegroup, walltype.Id)
        t.Commit()
        revitcomid= UI.RevitCommandId.LookupPostableCommandId(UI.PostableCommand.ArchitecturalWall)
        if revitcomid:
...
def firstrun():
    ...
    walltypes = FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Walls) \
                             .WhereElementIsElementType().ToElements()
    genmod2 = [i for i in walltypes if not i.FamilyName.Equals("Model Text")]
...
class ModelessForm(WPFWindow):
    ...
    def __init__(self, xaml_file_name, buttonlist):
        ...
        self.text1.Text = "Walltypes (#" + str(len(typelist)) + ")"
