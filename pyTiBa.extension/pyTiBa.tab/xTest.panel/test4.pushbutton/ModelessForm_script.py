...
class SelectFromList(TemplateUserInputWindowModeless):
    ...
    Example:
        ...
        [True, False, True] 
    """
    def set_states(self, state=True, flip=False, selected=False): 
    ...
    def check_selected(self, sender, args):    #pylint: disable=W0613
        ...
        self.set_states(state=True, selected=True) 

    def uncheck_selected(self, sender, args):    #pylint: disable=W0613
        """Mark selected checkboxes as unchecked."""
        self.set_states(state=False, selected=True) 
...
#doc.ActiveView.SetFilterVisibility(elementid, True) 


ops = [option(x) for x in filter] 


res = SelectFromList.show(sorted([option(x) for x in filter], key= lambda x: x.name), 
                                multiselect=True,  
                                return_all = True, 
